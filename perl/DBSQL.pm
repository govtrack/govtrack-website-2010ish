...
sub Open { # ($dbname, $username, $password)
	...
	if (defined($dbh)) { return; } # { die "Database already open."; }
...
sub CheckOpen {
	if (!defined($dbh)) {
		...
		die "Database not open: " . $trace->as_string;
	}
}
...
sub SpecContains { return Spec($_[0], 'like', '%' . Escape($_[1], 1) . '%', 1); }
sub SpecStartsWith { return Spec($_[0], 'like', Escape($_[1], 1) . '%', 1); }
sub SpecEndsWith { return Spec($_[0], 'like', '%' . Escape($_[1], 1), 1); }
